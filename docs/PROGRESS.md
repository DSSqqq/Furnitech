...
**Последнее обновление:** 2026-06-28 — шаг 8: удаление фасада в футере, без «Изменить» в тулбаре.


### Изменения 2026-06-28 (frontend — шаг 8: футер и удаление фасада)

#### Сводка

Убрана панель **`step8-facade-toolbar`** под переключателем фасадов: кнопки **«Изменить»** и **«Удалить»** удалены из этой зоны. Удаление перенесено в **`step8-result__footer-actions`**: **«Удалить текущий»** рядом с **«Новый расчёт»**, стиль **`admin-primary`** (как основное действие футера). Редактирование фасада — через навигацию шагов калькулятора, без отдельной кнопки на шаге 8.

#### Изменения

- **`Step8FrameResult.tsx`**: удалены `onEditSelectedFacade` и блок `step8-facade-toolbar`; кнопка удаления в футере.
- **`Step8FrameResult.css`**: удалены стили тулбара фасада; правила печати без `step8-facade-toolbar`.
