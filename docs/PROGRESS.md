...
**Последнее обновление:** 2026-05-24 — доработка режима «только десктоп»: фиксированная геометрия при zoom, один scroll документа, sticky-папки в справочниках, двухколоночная сетка «Расчёты»; ранее — viewport **1280**, эскиз **500×200** при нулевых габаритах.
...
- **`frontend/index.html`:** **`meta viewport`** — **`width=1280`**, **`user-scalable=yes`**, **`minimum-scale=0.25`**, **`maximum-scale=5`**. Брейкпоинты **`@media (max-width: 1024px)`** не срабатывают; на телефоне — полноразмерная версия с **pinch-zoom**.
