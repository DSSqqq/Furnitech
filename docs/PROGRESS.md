...
|--------------|------------|
| `AdminApp.tsx` | Дерево `TreeRow`, папки: **шестерёнка** → меню «Переименовать» / **«Удалить…»**; удаление папки — **модалка** с текстом о **каскадном** удалении (`createPortal` в `document.body`), API **DELETE** с каскадом на бэкенде; сброс выбранной папки при попадании в удалённое поддерево. `MaterialForm`: удаление материала — **модалка** с предупреждением (не `window.confirm`). Портал `MaterialExtrasPanel`. |
