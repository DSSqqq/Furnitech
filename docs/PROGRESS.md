...
- **Админка:** кнопка **«Поиск»** у дерева папок — только **`mode="navigate"`**; текст **`HintButton`** у блока папок обновлён под сценарий «выделить строку → Перейти».
...
- **`AdminApp.tsx`:** в **`FolderMoveModal`** передаются **`onMove={applyFolderMove}`** и **`onMoveMaterial={applyMaterialMove}`** (обновление списка материалов и карточки при переносе открытого материала).
...
|--------------|------------|
| `AdminApp.tsx` | Шапка: **`admin-header-top`**, **`admin-section-tabs`**. Дерево **`TreeRow`** (классы **`folder-explorer-*`** на строках); **`FolderMoveModal`**: **`applyFolderMove`**, **`applyMaterialMove`** (**`onMoveMaterial`**); **`FolderCreateModal`**, **`MaterialSearchModal`** (**`navigate`**); **`MaterialForm`**; **`MaterialExtrasPanel`**. **«Заказы»** — **`AdminOrdersPanel`**. |
...
| `FolderMoveModal.tsx` | Explorer: DnD **любой** папки (дерево, **«Все папки»**, плитка 📁); опционально DnD материала 📄 (**`onMoveMaterial`** → **`PATCH`** материала **`category`**); **`executeFolderMove`** без **`onClose`** после успеха; кнопка **«Закрыть»**; типы DnD **`DND_FOLDER`** / **`DND_MATERIAL`**. |
| `MaterialSearchModal.tsx` | Модалка поиска: фильтры, дерево, таблица. Режимы: **`multiPick`** (чекбоксы, накопление выбора, **«выбрать все»**, **`onPick`**) — калькулятор; **`navigate`** (подсветка строки, **«Перейти»**, **`onNavigate`**) — админка материалов. **`fetchMaterialsFiltered`**, debounce. |
...
- **Папки:** клик по **названию** — выбор; **▸/▾** — свернуть/развернуть; **⚙** — переименовать, **переместить** (**`FolderMoveModal`**: DnD любой папки / материалов 📄 при **`onMoveMaterial`**, **«Закрыть»** по окончании), удалить (модалка + **DELETE**). **Создание** — **`FolderCreateModal`**. **Поиск** — **`MaterialSearchModal`** (**navigate**).
...
- [x] Поиск материалов в админке: **`MaterialSearchModal`** (**`navigate`**: **«Перейти»** к карточке); в калькуляторе — **`multiPick`**; гибкий поиск на бэкенде (**rapidfuzz** + многословные запросы).
