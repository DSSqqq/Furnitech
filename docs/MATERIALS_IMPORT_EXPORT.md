...
- **Текстура и база (`TextureItem`):** если в **`Texture`** (путь к файлу) по **stem** последнего сегмента найден объект в базе — карточка в режиме **«текстура»**, **`texture_item`** выставлен. Если колонка **`Текстура`** пуста, дополнительно смотрится строка **`Comment` / При** на фрагмент **`Текстура (импорт):`** (формат типовых сокращённых выгрузок Modus и т.п.) и путь после двоеточия — его разбираем так же, как колонку **«Текстура»**. Если текстуры по имени **нет**, остаётся **«цвет»**, **`texture_color`** из **HEX/RGB**.
- При **экспорте** если у материала задан **`texture_item`**, в **`Texture`** пишется **наименование из базы**; иначе — значение из снимка строки (**`import_export_snapshot`**, как раньше).
