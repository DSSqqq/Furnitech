...
- **Шаг 2:** зависит от фасада; для рамочного — **тип профиля** (`CalculatorProfileType`, до **трёх** изображений карточки + цвета-материалы) и **цвет** (`Step2FrameFacade`). Заголовок **`frame3-title`** — **«Выберите тип профиля и цвет»**; **«+ Добавить тип профиля»** — в той же строке **`admin-heading-row calc-card-title-row`**, не в отдельном **`frame2-card-head`**. Меню ⚙ на плитке — **`TileGearMenu`** (portal на **`document.body`**, **`tree-gear-menu--portal`**). См. таблицу «Калькулятор (UI)» — строки **«Карточка типа профиля (фото)»**, **«Меню плитки (⚙)»**.
...
- **Шаг 4 (только рамочный):** наполнение — `…/frame/filling` (`Step4FrameFilling`); **`frame3-title`** — **«Выберите тип наполнения»**; **«+ Добавить тип наполнения»** — в **`admin-heading-row calc-card-title-row`** (как шаг 2). Меню ⚙ — **`TileGearMenu`**. У **`CalculatorFillingType`** до **трёх** файлов карточки (**`0036`**) — тот же UI-паттерн, что шаг 2.
...
| `calculator/Step2FrameFacade.tsx` | Шаг 2 (рамочный): **`frame3-title`** + **`admin-heading-row`** с **«+ Добавить тип профиля»**; плитки — **`CalculatorCardTileStriped`**; ⚙ — **`TileGearMenu`**. Формы: **`ProfileCardImageTileRow`**, **`MaterialSearchModal`**. Эскиз **`facadeSketchBoxStyle`**. |
