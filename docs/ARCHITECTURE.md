...
| Ориентировочная цена | Панель **`CalcPriceTotals`** справа (`CalculatorPage.tsx` + **`CalculatorPage.css`**: `.calc-body-with-totals`, `.calc-totals-*`). На **шагах 1–2** только подсказка, суммы нет (цена появляется с шага 3 после ввода габаритов). Подписка на **`calc-frame-session`** и `storage`, снимок ключей через **`readCalculatorPriceConfigKey`** (`frameCalcSession.ts`). Данные: `fetchMaterial` по `calc_frame_color_id` и при необходимости `calc_filling_material_id` (в ответе — **`pricing_calc_mode`**). Подпись «в расчёте как …» — **`resolveMaterialPricingUomCode`**. При выборе типа фасада на шаге 1 вызывается **`clearFrameCalculatorStorage()`**, чтобы не подтягивать прошлую конфигурацию. |
...
| `calculator/framePriceEstimate.ts` | Ориентировочная стоимость: **`resolveMaterialPricingUomCode`**, **`pricingCalcModeToUomCode`**, **`unitsPerFacade`**, **`pricedUnitsForMaterial`**, **`materialLineCost`**, **`relatedItemsCalculatorCost`**, **`relatedItemCalculatorCost`**, **`computeFramePriceBreakdown`**. |
| `calculator/calculationFormula.ts` | Безопасная оценка активной формулы: токены классов превращаются в стоимость выбранных клиентом материалов с этим классом; объём через **`materialLineCost`** / **`pricedUnitsForMaterial`** (учитывает **`pricing_calc_mode`**). |
| `calculator/CalcPriceTotals.tsx` | UI итога; шаг 1 без цифр; fallback габаритов — **`FRAME_DEFAULT_*`**; подпись ед. изм. в расчёте — **`resolveMaterialPricingUomCode`**. |
