...
Панели **`#admin-panel-orders`** и **`#admin-panel-users`** используют тот же tabpanel-корень **`AdminPanelLoadingHost`**, что и справочники. На десктопе их зона растягивается CSS-ом до доступной высоты под шапкой, а внутренняя **`.admin-orders-placeholder`** занимает всю ширину tabpanel; DOM таблиц и логика загрузки при этом не меняются.
...
| Эскиз | Общий вид `.sketch`: отрисовка текстуры через **`profileFrameTextureLayerStyle`** / **`materialTextureLayerStyle`** (`sketchFrame.ts`) на внутренних слоях `.sketch-frame-texture` и `.sketch-paper-texture`. **Рамка профиля:** **`useFrameColorMaterial`** — summary из **`calculator-profile-types`** + **`fetchMaterial`**, как на шаге 2; **`profileFrame`** — opacity **1** (не **`tex_opacity`** карточки). **Наполнение:** **`materialFillingTextureLayerStyle`** — фиксированная непрозрачность **`SKETCH_FILLING_TEXTURE_OPACITY` (0.18)**, не **`tex_opacity`** материала. Пропорции — **`facadeSketchAspectRatio`**, вертикальный масштаб — **`facadeSketchScaleY`** / **`facadeSketchBoxStyle(H_mm, W_mm)`** (шаги 2–7). **`--sketch-scale-y`** не растёт выше **`SKETCH_SCALE_HEIGHT_CAP_MM` (1700)**. **Шаг 3:** для чертежа **`frameSketchDisplayDims`** — пустое поле или **0** → **500×200** мм по оси; иначе введённые габариты. Лист **`.sketch-sheet`**: чёрный текст, уменьшенный кегль (**`Step2FrameFacade.css`**). |
...
| Шаг 8 «Итог» (вёрстка) | **`calc-routes-wrap--step8`** (публичный **`#public-panel-calculator`** и админ **`#admin-panel-calculator`**). Десктоп: **`.frame2.step8-result`** — **grid** две колонки (контакты | детали); **`Step8FrameResult.css`** задаёт шаблон, **`CalculatorPanelShell.css`** (≥1025px) не должен переключать контейнер на **`flex-direction: column`** (иначе детализация уезжает под форму). Слева **`step8-result__contact`**, справа **`step8-result__scroll-pack`** (детализация, таблица цен, **«Добавить фасад»**). **Прокрутка (≥1025px):** единая **страница (`html`)**, не внутренние колонки — у **`.step8-result__scroll-pack`** и **`.calc-side-panel-scroll`** на шаге 8 **`overflow: visible`**; при **`:has(.calc-routes-wrap--step8)`** снят **`overflow: hidden`** у цепочки **`.calc-panel-shell`** / **`.admin`** / **`.public-shell`** (Chrome иначе глотает `wheel`). Шаги 1–7 — scroll в **`.calc-routes-wrap`** / **`.calc-side-panel-scroll`**. ≤1024px — одна колонка. |
...
| Альтернативный расчёт «по классам» | **`GET /api/calculation-formulas/?active=1`**, первая **активная** подходящая формула (**`matchFormulaTotalForFrame`**, **`is_active`**). **`evaluateCalculationFormulaWithBreakdown`** — итог + расшифровка **`CalcPriceBreakdownView`** по классам и строкам материалов. Подробно: **[CALCULATION_FORMULAS.md](CALCULATION_FORMULAS.md)**. |
...
- **`rounding_mode`** / **`rounding_multiple`**: округление количества вверх после избытка (**`applyQuantityRounding`** в **`priceBreakdown.ts`**).
...
| `calculator/priceBreakdown.ts` | **`MaterialLineBreakdown`**, **`collectFrameMaterialLines`** (профиль, наполнение, петли), **`buildHingeMaterialLine`**, **`applyQuantityRounding`**, **`serializePriceBreakdownForSnapshot`**. |
| `calculator/framePriceEstimate.ts` | **`computeFramePriceBreakdown`** (+ **`hinges`**); **`pricedUnitsForMaterial`**; **`collectCurrencies`** с петлями. |
| `calculator/calculationFormula.ts` | Формулы по классам; **`hingeSubtotalOutsideFormula`**; **`matchFormulaTotalForFrame`** (**`is_active`**, петли в дереве классов). |
