...
| Эскиз | Общий вид `.sketch`: отрисовка текстуры через **`materialTextureLayerStyle`** (`sketchFrame.ts`) на внутренних слоях `.sketch-frame-texture` и `.sketch-paper-texture`. Пропорции — **`facadeSketchAspectRatio`**, вертикальный масштаб — **`facadeSketchScaleY`** / **`facadeSketchBoxStyle(H_mm, W_mm)`** (шаги 2–7). **`--sketch-scale-y`** не растёт выше уровня **`SKETCH_SCALE_HEIGHT_CAP_MM` (1700)** — при больших H эскиз не увеличивается по высоте, пропорции (aspect) от реальных габаритов. Габариты для эскиза: из сессии (**`readFrameDimsMm`**) или дефолт материала (**`frameDimDefaultsFromMaterial`**: min материала или 500×200). Лист **`.sketch-sheet`**: чёрный текст, уменьшенный кегль; таблица **`.sketch-row`** — колонка ключа **5rem**, gap **0.25rem** (**`Step2FrameFacade.css`**). На шагах 2 и 4 для `tex_*` догружается **`fetchMaterial(id)`**. |
...
| Левая панель формы | **`.calc-side-panel`** (`CalculatorPage.css`): колонка **flex** фиксированной высоты (**`--calc-side-h`**), **`overflow: hidden`**; прокрутка только в **`.calc-side-panel-scroll`** (шапка и **`frame2-card-nav`** остаются видимыми). Шаги 1–8 используют этот паттерн. **Ширина** панели на десктопе: **`min(var(--calc-steps-tabs-width), 100%)`** — переменная задаётся JS (**`syncCalcStepsTabsWidth`** в **`CalculatorPage.tsx`**) по ширине полосы вкладок до «Итог». |
| Шаг 8 «Итог» (вёрстка) | В админке — **`#admin-panel-calculator`**, **`calc-routes-wrap--step8`**. Левая колонка контактов: **`step8-result__contact frame2-card calc-side-panel`**, форма в **`calc-side-panel-scroll`**, нижняя полоса — **`frame2-card-nav`**. Справа **`step8-result__scroll-pack`** — прокрутка детализации и кнопки **«Добавить фасад»** (**`Step8FrameResult.css`**, **`AdminApp.css`**). |
...
| `/api/calculator-profile-types/`, `/{id}/` | CRUD | Типы профилей + цвета (материалы) + флаги + `card_image` … `card_image_4`. **GET — анонимно; запись — JWT + model perms.** |
| `/api/calculator-filling-types/`, `/{id}/` | CRUD | Типы наполнения + материалы + **`card_image` … `card_image_4`** (шаг 4). **GET — анонимно; запись — JWT + model perms.** |
| `/api/calculator-hinge-types/`, `/{id}/` | CRUD | Типы петель + материалы + **`card_image` … `card_image_4`** (шаг 5). **GET — анонимно; запись — JWT + model perms.** |
...
| Модуль | Роль |
|--------|------|
| `App.tsx` | `/login` (**`LoginRoute`**, **`safePostLoginTarget`**); **`AdminApp`** на `/materials/*`, `/textures/*`, `/uom/*`, `/classes/*`, `/calculations/*`, `/calculator/*`, `/orders/*`, `/users/*` (**`AdminRoute`**); родитель **`/`** — **`PublicShell`** с вложенными маршрутами: индекс и **`*`** — **`CalculatorPage variant="public"`**, **`my-orders`**, **`guide`** → **`/`**. |
...
| `calculator/frameCalcSession.ts` | **`FRAME_DEFAULT_HEIGHT_MM`**, **`FRAME_DEFAULT_WIDTH_MM`** (500 / 200); **`FRAME_DIM_FALLBACK_MAX_MM`** (99999); **`frameDimDefaultsFromMaterial`**, **`hasSavedFrameDims`**, **`seedFrameDimsFromMaterial`**, **`effectiveFrameDimMax`**, **`clampFrameDimDigits`**; `isFrameStep2Ready`, **`isFrameStep4Ready`**, **`isFrameMortiseHingeSelected`**, события сессии, **`readFrameDimsMm`**, ключи присадки/петель/ручки, хелперы петель и ручки. |
...
| `calculator/sketchFrame.ts` | `resolveMediaUrl`, **`facadeSketchBoxStyle`**, **`facadeSketchScaleY`**, **`facadeSketchAspectRatio`**, **`SKETCH_SCALE_HEIGHT_CAP_MM`**, **`materialTextureLayerStyle`**, `sketchFrameInlineStyle`. |
...
| `index.css` / `App.css` / `AdminApp.css` | Десктоп: скролл внутри колонок / **`public-shell__main`**. **`CalculatorPage.css`**: **`calc-side-panel`**, **`--calc-steps-tabs-width`**, **`calc-totals-inline`**. **`#admin-panel-calculator`**: компактный UI, flex-скролл шага, **`tree-gear-menu--portal`**, шаг 8 — **`step8-result__scroll-pack`**. |
