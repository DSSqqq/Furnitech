...
- **Временно (2026-05-24):** адаптив **`mobile.css`** **отключён**; вместо него **`desktop-layout.css`** + **`viewport width=1280`** в **`index.html`** — макет всегда десктопный, прокрутка документа по X/Y (только **`html`**, без дубля на **`body`**); на телефоне pinch-zoom. Late overrides в **`desktop-layout.css`** фиксируют desktop-сетки при browser zoom (без **`dvh`** у эскиза/панелей). В справочниках (**материалы**, **текстуры**, **классы**, **расчёты**, **`MaterialSearchModal`**) левая колонка **`.admin-aside`** — **`position: sticky`** для DnD в папки при scroll списка; вкладка **«Расчёты»** — двухколоночная сетка как у материалов. См. [PROGRESS.md](PROGRESS.md).
