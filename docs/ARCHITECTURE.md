...
| `calculator/Step2FrameFacade.tsx` | Шаг 2 (рамочный): плитки, модалка цвета, эскиз с **`facadeSketchBoxStyle`** и подпиской на сессию (**`useSyncExternalStore`**, снимок **`"h|w"`**); при **`readOnly`** скрыт CRUD типов и шестерёнки. |
...
- **Шестерёнка (⚙):** по **наведению** на строку появляется; меню: **Переименовать** (inline-редактирование), **Удалить…** — далее **модальное окно** (портал в `body`, не нативный `confirm` — надёжнее в WebView/встраиваемых браузерах) с предупреждением о **каскадном** удалении вложенных папок и материалов; только после **«Удалить»** в модалке уходит `DELETE` на API (**каскад на бэкенде**, см. [категории](#категории-материалы-папки)).
