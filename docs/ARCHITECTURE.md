...
| Чертёжные размеры (шаг 3) | Блок **`frame3-drawing`** обнимает `.sketch`; **`frame3-dim-drawing`**: выносные пунктирные линии, размерная линия со стрелками, подписи мм; `z-index` выше листа эскиза. Зазор подпись ↔ линия — **`--frame3-dim-label-gap`** (8px) на **`.frame3-drawing`**, для габаритов через **`gap`** у **`--top`/`--bottom`** и **`__left-col`**. Модификаторы **`--right`** / **`--bottom`** задают расположение основного габарита **напротив** стороны с петлями, чтобы линии не накладывались. |
| Петли на эскизе (шаг 5–6) | Маркеры у **внешнего** края: класс **`sketch--hinge-markers`**, подписи **№n** — **`.sketch-hinge-pin-label`**, цвет **`--ft-canvas-hinge-label`** (чёрный). **Шаг 6:** каждая петля — **один** выносной размер **от края фасада до петли** (№1 от начала стороны, №2…№n от противоположного края). Слой **`frame3-hinge-dim-layer`**, блоки **`.hinge-chain-dim`** с **`trackTopPct`** / **`trackSpanPct`** (`top` + `height` или `left` + `width`); **`trackOffsetPx`**: **№1** и **№n** — **0**, **№2…№n−1** — ступени **1…n−2** × **26px**; выноски **`--hinge-chain-track-offset`**. Подписи: **`--hinge-chain-label-gap`** ← **`--frame3-dim-label-gap`**. **Верх/низ:** **`__val--h`** над/под линией. **Лево/право:** **`translate(±50%, -50%) rotate(±90deg)`** по центру **`.hinge-chain-dim__v`**. Класс **`--tracked`** только для DOM (ступеньки выносок); отдельных override подписи нет. **`--narrow`** — без стрелок на коротком сегменте. |
...
- **Тема оформления:** **`dark`** (по умолчанию) и **`light`** — CSS-переменные **`--ft-*`** в **`index.css`**, атрибут **`data-theme`** на **`<html>`**, переключатель **`ThemeToggle`** в **`admin-header-top`** (сохранение **`furnitech-theme`** в **`localStorage`**). См. **`theme.ts`**, **`ThemeProvider.tsx`**.
...
| `calculator/Step6FrameHingeLayout.tsx` | Шаг 6: **`frame3-title`** «Расстояния»; **2…10** отверстий (**`countStr`**, clamp на blur); ввод **№1 от начала / №2+ от конца**; **`hingeChainDims`** — **`trackTopPct`**, **`trackSpanPct`**, **`trackOffsetPx`** (№1 и №n — 0); эскиз **от края до петли**; редирект при **`!isFrameMortiseHingeSelected()`**. |
