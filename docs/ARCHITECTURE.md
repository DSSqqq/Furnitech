...
| `calculator/frameClientPdf.ts` | PDF шага 8 (**jspdf**, **jspdf-autotable**): **один лист** A4 **альбом** — слева бланк заказа (таблицы, стоимость, подписи внизу), справа **примерный эскиз** (**`drawFacadeSketchInArea`**); отдельные страницы «Фасад № N» **не добавляются**. Строка «Раскладка петель» в таблице — только если **`includeHingeLayoutRow`**. Noto Sans TTF — кэш base64 + **`addFileToVFS`** / **`addFont`** на каждый **`jsPDF`**; **`/fonts/NotoSans-Regular.ttf`** или CDN. |
