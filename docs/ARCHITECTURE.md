...
- **Временно (2026-05-24):** адаптив **`mobile.css`** **отключён**; вместо него **`desktop-layout.css`** + **`viewport width=1280`** в **`index.html`** — макет всегда десктопный, прокрутка документа по X/Y; на телефоне pinch-zoom. См. [PROGRESS.md](PROGRESS.md).
