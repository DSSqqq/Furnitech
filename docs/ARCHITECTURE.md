...
| `calculator/frameClientPdf.ts` | PDF шага 8 (**jspdf**, **jspdf-autotable**): **один лист** A4 **альбом** — слева бланк, справа эскиз (**`drawFacadeSketchInArea`**, **`dimsOutsideSketch`** — размеры снаружи, рамка не сжимается). Цепочки и габариты — **`pdfSketchDimsDraw.ts`**; текстуры — **`pdfDrawMaterialTextureLayer`** (профиль + наполнение, как **`materialTextureLayerStyle`** / **`SKETCH_FILLING_TEXTURE_OPACITY`**). Noto Sans — **`addFileToVFS`** на каждый документ. |
| `calculator/pdfSketchDimsDraw.ts` | Размеры в PDF: цепочки (**`CHAIN_DIM_OFFSET_FRAC`**, выноска = линия), подписи (**`pdfMainLabelPadMm`** для H×W; **`pdfChainVertHingeLabelPadMm`** / **`pdfChainLabelPadMm`** для петель и ручек, аргумент **`chainKind`**). |
