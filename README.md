...
2. **Фронт** (другой терминал):
   ...
   **Главная страница (`/`)** — публичный **калькулятор** (подбор фасада без входа: только выбор вариантов, без создания/редактирования справочников); в шапке вкладки **Калькулятор** и **Мои заказы** (`/my-orders`, заглушка до API заказов). Регистрация: **`/register`**. **Вход** — `/login`; после входа **сотрудник** (`is_staff`) открывает **`/materials`**, **`/calculator`**, **`/orders`**, **`/users`** (веб‑админка в `AdminApp`); **клиент без staff** остаётся в публичной зоне (калькулятор).

   В шапке веб‑админки (после входа сотрудника) полоса разделов в стиле публичных вкладок: **Материалы**, **Калькулятор**, **Заказы** (заглушка), **Пользователи**.
